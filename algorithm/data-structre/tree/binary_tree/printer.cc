...
typename Printer::InfoNode* Printer::initTreeStructure(void* node,
                                                       InfoNode* parent) {
...
void Printer::initPosInfo() {
  if (this->root == nullptr || this->layerHead.empty()) return;
  this->root->pos = 0;
...
void Printer::movePos(InfoNode* node, int len) {
  if (node->left) this->movePos(node->left, len);
  if (node->right) this->movePos(node->right, len);
  node->pos += len;
}
...
class IamaTree : public Printer {
  void* getRoot() const { return (void*)1; }
